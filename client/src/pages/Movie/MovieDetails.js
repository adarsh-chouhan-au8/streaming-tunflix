...
const MovieDetails = ({ title, vote_average, genres, runtime, year, overview }) => {
        ...
        <Grid container spacing={2} style={{margin:'10vh 0'}}>
            ...
            <Grid item xs={12} className={classes.desc}>
                ...
                {genres.slice(0, 3).map((elem,k) => (
                    <Paper key={k} className={classes.infos}>{elem.name}</Paper>
                ))}
                <Paper className={classes.infos}>{year}</Paper>
                <Paper className={classes.infos}>
                    {`${Math.floor(runtime / 60)} h ${runtime - Math.floor(runtime / 60) * 60} min`}{" "}
                </Paper>
            </Grid>
            <Grid item xs={12}>
