...
const fetchTMDB = async (id) => {
    try {
        const [{ data }, requestTrailer] = await axios.all([
            axios.get(
                `http://api.themoviedb.org/3/tv/${id}?api_key=${config.TMDBApiKey}&append_to_response=credits&language=en`
            ),
            axios.get(`http://api.themoviedb.org/3/tv/${id}/videos?api_key=${config.TMDBApiKey}&language=en`),
        ]);
        ...
        let seasons = await axios.all(
            [...Array(number_of_seasons).keys()].map((k) =>
                axios.get(
                    `https://api.themoviedb.org/3/tv/${id}/season/${k + 1}?api_key=${config.TMDBApiKey}&language=en`
                )
            )
        );
        seasons = seasons
